...
const StyleButton = Styled.TouchableOpacity`padding: 8px;`;
...
const MovieHome = ({navigation}: Props) => {
  ...
  return (
    <Container>
      ...
      <SubCatalogList
        title="최신 등록순"
        url="https://yts.lt/api/v2/list_moives.json?sort_by=date_added&order_by=desc&limit=10"
        onPress={(id: number) => {
          navigation.navigate('MovieDetail', {id});
        }}
      />
      <SubCatalogList
        title="평점순"
        url="https://yts.lt/api/v2/list_moives.json?sort_by=rating&order_by=desc&limit=10"
        onPress={(id: number) => {
          navigation.navigate('MovieDetail', {id});
        }}
      />
      <SubCatalogList
        title="다운로드순"
        url="https://yts.lt/api/v2/list_moives.json?sort_by=download_count&order_by=desc&limit=10"
        onPress={(id: number) => {
          navigation.navigate('MovieDetail', {id});
        }}
      />
